...
from common import (rjson, wjson, VERSION, VERSION_TEXT, MIN_ENGINE, UUID,
                    CORE_BP, CORE_RP, GUNS_BP, GUNS_RP, LINK_BP, BODY, body_factor)

# ------------------------------------------------------------------------------------------ manifests
LINK_VERSION = [1, 0, 2]   # its own number: bump when scripts/main.js of the Link BP changes (1.0.2: food drain)
...
def manifests(out):
    ...
    core_bp = {
        ...
        "dependencies": [
            ...
            {"uuid": UUID[LINK_BP][0], "version": LINK_VERSION},
    ...
    link_bp = {
        ...
        "header": {"name": "Succubi Server Link BP",
                   ...
                   "uuid": UUID[LINK_BP][0], "version": LINK_VERSION, "min_engine_version": MIN_ENGINE},
        "modules": [
            {"type": "data", "uuid": UUID[LINK_BP][1], "version": LINK_VERSION},
            {"type": "script", "language": "javascript", "entry": "scripts/main.js", "uuid": UUID[LINK_BP][2], "version": LINK_VERSION}],
...
SPEED_STEPS = list(range(140, 221, 10))     # the player range; admins beyond it get the edge speed


def speed_groups(e):
    """walking speed by height, as its own group family (succubi:spd_XXX, XXX = cm) so the scripts can switch it off
    (the "height changes speed" setting) without touching body size. Same event shape as kotarus:set_h_XXX:
    drop the other speed groups, add this one."""
    names = [f'succubi:spd_{cm:03d}' for cm in SPEED_STEPS]
    for cm, name in zip(SPEED_STEPS, names):
        e['component_groups'][name] = {"minecraft:movement": {"value": round(BODY['walk'] * body_factor(cm, BODY['speed_short'], BODY['speed_tall']), 4)}}
        e['events'][f'succubi:set_spd_{cm:03d}'] = {"remove": {"component_groups": [n for n in names if n != name]},
                                                     "add": {"component_groups": [name]}}
    return len(names)


def body_js():
    """the same numbers for the scripts (HP, hit strength, the height board)"""
    return ('// GENERATED by tools/v20/packs.py from common.BODY - height -> max HP, speed and hit strength\n'
            f'export const BODY = {json.dumps(BODY)};\n'
